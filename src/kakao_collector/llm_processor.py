...
2. 메타데이터(YAML): 반드시 본문 최상단에 아래 형식의 YAML Front Matter를 포함해야 합니다.
   ...
   - tags: 업무일지, 거래처명/담당자 외에 대화에서 언급된 '브랜드명'이나 '주요 상품명'을 배열로 기재합니다. [절대 규칙] 옵시디언 태그 인식 오류를 방지하기 위해 태그 문자열 내의 띄어쓰기는 반드시 언더바(_)로 치환하고, 쉼표나 콜론 등 특수문자는 모두 제거하세요. (예: "Gold Series" -> "Gold_Series")
...
tags: [업무일지, {partner_tags}, 여기에 브랜드명과 모델명을 쉼표로 구분하여 삽입]
---
...
- 마크다운 작성 시 일반 기호로 `*`를 사용할 때는 반드시 `\*`로 이스케이프 처리하세요. (예: `\*재입고 표시 모델`). 단독 `*` 기호로 인해 표(Table) 등의 마크다운 렌더링이 깨지지 않도록 각별히 주의하세요.
"""
...
def generate_journal_markdown(partner: str, date_str: str, chat_text: str, links_text: str, doc_list: list[str], media_paths: list[Path], excel_contents: dict[str, str] | None = None) -> str:
...
"""
    ...
    partner_tags = ", ".join(partner.replace(":", "_").split("_"))

    response = client.messages.create(
        model="claude-sonnet-4-6",
        max_tokens=2048,
        temperature=0.1,
        system=SYSTEM_PROMPT.format(date=date_str, partner=partner, partner_tags=partner_tags),
        messages=[
