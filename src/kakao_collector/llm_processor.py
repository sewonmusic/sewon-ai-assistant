...
3. 정보 추출 지침 (해당하는 이벤트가 있을 경우, 관련된 모든 데이터 수치, 이름, 주소, 연락처 등을 절대 생략하지 말고 구체적인 표나 리스트 형식으로 기재하세요):
...
반드시 문서 맨 위와 YAML 끝부분에 '---' 기호를 빠짐없이 적어야 합니다.
...
# 📦 세부 추출 데이터
식별된 이벤트 유형과 구체적인 데이터를 아래 예시와 같이 마크다운 표로 작성하세요.
| 상품명 | 수량 | 기타 특징 |
|---|---|---|
| grg121dx wnf | 2대 | 발주 |

# ✅ 할 일 (Action Items)
- [ ] 추출된 일정이나 해야 할 일 작성 (없으면 "해당 없음")

# 📎 첨부파일 및 미디어 분석
- 언급된 파일이나 공유된 이미지 이름 및 OCR/이미지 분석 내용 요약

# ❓ 미해결 이슈
- 확인이 필요한 사항 작성 (없으면 "해당 없음")
